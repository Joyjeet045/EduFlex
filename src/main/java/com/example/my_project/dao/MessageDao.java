...
public class MessageDao {
    ...
    public int save(Message message) {
      ...
      return jdbcTemplate.update(sql, message.getChatRoom().getId(), message.getSender().getId(), message.getContent(), message.getTimestamp());
    }
    public List<Message> findByChatRoomId(Long chatRoomId) {
      String sql = "SELECT * FROM messages WHERE chatroom_id = ?";
      return jdbcTemplate.query(sql, new Object[]{chatRoomId}, new BeanPropertyRowMapper<>(Message.class));
    }
